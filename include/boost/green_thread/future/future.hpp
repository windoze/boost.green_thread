...
#ifndef BOOST_GREEN_THREAD_FUTURE_FUTURE_HPP
#define BOOST_GREEN_THREAD_FUTURE_FUTURE_HPP
    ...
    namespace detail {
        ...
        struct any_state : std::enable_shared_from_this<any_state<Ret>> {
            ...
            void set_value(Ret v) {
                boost::unique_lock<mutex> lk(mtx);
                flag=true;
                ret=v;
                cv.notify_all();
            }
        ...
        template<typename F>
        struct all_waiter<F> {
            static void setup(condition_variable &cv, boost::atomic<size_t> &acquired, size_t count, F &f) {
        ...
        struct all_waiter<F, Futures...> {
            static void setup(condition_variable &cv, boost::atomic<size_t> &acquired, size_t count, F &f, Futures&... fs) {
