...
#ifndef BOOST_GREEN_THREAD_IOSTREAM_HPP
#define BOOST_GREEN_THREAD_IOSTREAM_HPP
...
namespace boost { namespace green_thread {
    ...
    struct listener {
        ...
        // Start and join, other thread may stop the listener
        template<typename F>
        ...
        void start(F f) {
            if(!stop_signal_) {
                ...
                if(!acceptor_thread_) {
                    acceptor_thread_.reset(new thread(&listener::acceptor_thread<F>,
                                                    this,
                                                    ep_,
                                                    stop_signal_.get(),
                                                    f));
    ...
    private:
        template<typename F>
        void acceptor_thread(const endpoint_type & e, promise<void> *p, F f) {
            acceptor_type acc(e);
            boost::system::error_code ec;
            thread watchdog(thread::attributes(thread::attributes::stick_with_parent),
                           &listener::acceptor_watchdog_thread,
                           this,
                           p,
                           std::ref(acc));
            while(!ec) {
                ...
                acc(*s, ec);
                if(ec) break;
                thread([f](std::unique_ptr<stream_type> str){
                    f(*str);
                }, std::move(s)).detach();
            }
            watchdog.join();
        ...
        void acceptor_watchdog_thread(promise<void> *p, acceptor_type &acc) {
            p->get_future().wait();
            acc.close();
        }
        ...
        endpoint_type ep_;
        std::unique_ptr<thread> acceptor_thread_;
